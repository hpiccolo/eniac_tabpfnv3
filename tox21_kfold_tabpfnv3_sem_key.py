...
SEED = 46785                  
N_FOLDS = 5                   
USAR_AMOSTRA = False         # Altere para False quando for rodar o Tox21 cheio
TAMANHO_AMOSTRA = 500        
pasta_arquivos = 'C://'
...
print("\n" + "="*85)
print(f"🧪 EXPERIMENTO TOX21: CROSS-VALIDATION (5 FOLDS) | BR vs CC | SEED: {SEED}")
print("🧪 Abordagem: Matriz Coesa (Linhas com NaNs removidas)[cite: 1]")
print("="*85)

# 1. CARREGAMENTO E LIMPEZA TOTAL (Cohesive Matrix)
df_bruto = pd.read_parquet(pasta_arquivos+'dataset_tox21_fingerprints.parquet')
X_cols = [str(i) for i in range(512)]
y_cols = [col for col in df_bruto.columns if col not in X_cols]

# Mantém apenas moléculas com dados em todas as 12 vias (Matriz Densa)
df = df_bruto.dropna(subset=y_cols).copy()
print(f"✅ Base filtrada: {len(df)} moléculas com rótulos completos.[cite: 1]")
...
df_res.to_csv('TABPFNV3_resultados_Tox21_cv_metricas.csv', index=False)
print("✅ Concluído! Arquivo 'resultados_Tox21_cv_metricas.csv' salvo com a consolidação dos 5 Folds.")
